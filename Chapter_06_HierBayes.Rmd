...
```{r} 
data <- read.csv('data/Mosquito.csv', stringsAsFactors = F)
```
...
ggplot(data, aes(x = time, y = density, color = as.factor(replicate))) + 
  geom_point() + 
  geom_line() + 
  theme_classic()
...
"
data_jags <- list(X = data$density, 
                  mu0 = mean(data$density), 
                  S = 1/sd(data$density)^2,
                  T = 1/185) 
inits <- list() 
inits[[1]] <- list(mu = data_jags$mu0 + rnorm(1,0,data_jags$S))
inits[[2]] <- list(mu = data_jags$mu0 + rnorm(1,0,data_jags$S))
inits[[3]] <- list(mu = data_jags$mu0 + rnorm(1,0,data_jags$S))
...
burnin = 24000                                ## determine convergence
jags.burn <- window(var.out,start=burnin)  ## remove burn-in
plot(jags.burn)  

DIC <- dic.samples(model = j.model, 
            n.iter = 10000)
DIC
...
xpred <- seq(min(data$time),max(data$time), length.out = 100)  					## sequence of x values we're going to
...
for(g in seq_len(nsamp)){
  ...
  ycred[g,] <- theta["mu"] 
  ypred[g,] <- rnorm(npred,ycred[g,],1/data_jags$S)
}
...
plot(data$time,data$density,cex=0.5,ylim=c(6.5,8.5))
lines(xpred,ci[1,],col='green',lty=2)	## lower CI
lines(xpred,ci[2,],col='green',lwd=3)	## median
lines(xpred,ci[3,],col='green',lty=2)	## upper CI
lines(xpred,pi[1,],col='blue',lty=2)	## lower PI
lines(xpred,pi[2,],col='blue',lty=2)	## upper PI
