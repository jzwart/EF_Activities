...
y_all = gflu$Massachusetts
t = seq(1,length(y_all))
y = ifelse(t %% 4 == 0, y_all, NA) 
y_trun = ifelse(t < (max(t)-40), y_all, NA) 
plot(y ~ time,type='l',ylab="Flu Index",lwd=2,log='y')
```
...
data_all <- list(y=log(y_all),n=length(y_all),x_ic=log(1000),tau_ic=100,a_obs=1,r_obs=1,a_add=1,r_add=1)
data_trun <- list(y=log(y_trun),n=length(y_trun),x_ic=log(1000),tau_ic=100,a_obs=1,r_obs=1,a_add=1,r_add=1)
```
...
init_all <- list()
for(i in 1:nchain){
  y.samp = sample(y_all,length(y_all),replace=TRUE)
  init_all[[i]] <- list(tau_add=1/var(diff(log(y.samp))),tau_obs=5/var(log(y.samp)))
}
init_trun <- list()
for(i in 1:nchain){
  y.samp = sample(y_trun,length(y_trun),replace=TRUE)
  init_trun[[i]] <- list(tau_add=1/var(diff(log(y.samp))),tau_obs=5/var(log(y.samp)))
}
```
...
j.model   <- jags.model (file = textConnection(RandomWalk),
                             ...
                             inits = init,
                             n.chains = 3)
j.model_all   <- jags.model (file = textConnection(RandomWalk),
                             data = data_all,
                             inits = init_all,
                             n.chains = 3)
j.model_trun   <- jags.model (file = textConnection(RandomWalk),
                             data = data_trun,
                             inits = init_trun,
                             n.chains = 3)
```
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("tau_add","tau_obs"),
                                n.iter = 1000)
jags.out_all <- coda.samples (model = j.model_all,
                            variable.names = c("tau_add","tau_obs"),
                                n.iter = 1000)
jags.out_trun <- coda.samples (model = j.model_trun,
                            variable.names = c("tau_add","tau_obs"),
                                n.iter = 1000)
...
points(time, y_all, pch='+', cex=0.5, col = 'red')
...
```{r}
out_all <- as.matrix(jags.out_all)
x.cols <- grep("^x",colnames(out_all)) ## grab all columns that start with the letter x
ci_all <- apply(exp(out_all[,x.cols]),2,quantile,c(0.025,0.5,0.975))

plot(time,ci_all[2,],type='n',ylim=range(y_all,na.rm=TRUE),ylab="Flu Index",log='y',xlim=time[time.rng])
## adjust x-axis label to be monthly if zoomed
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
ecoforecastR::ciEnvelope(time,ci[1,],ci[3,],col="lightBlue")
ecoforecastR::ciEnvelope(time,ci_all[1,],ci_all[3,],col="blue")
points(time, y_all, pch='+', cex=0.5, col = 'red')
points(time,y,pch="+",cex=0.5)
```
* Generate a predicted (median) vs observed plot for the data points that were removed
```{r} 
xpred <- apply(exp(out[,x.cols]),2,median) 
xpred_all <- apply(exp(out[,x.cols]),2,median)

lims = range(range(c(y_all[is.na(y)]), range(xpred[is.na(y)])))
plot(y_all[is.na(y)] ~ xpred[is.na(y)],pch =16, ylim = lims, 
     xlim= lims, ylab= 'Obs', xlab= 'Pred')
abline(0,1)

```
* Comment on the accuracy and precision of the estimates.
```{r}
rmse <- sqrt(mean((y_all - xpred)^2)) # 
bias <- mean((y_all - xpred)^2)
r2 <- summary(lm(y_all ~ xpred))$r2
...
```{r}
out_trun <- as.matrix(jags.out_trun)
x.cols <- grep("^x",colnames(out_trun)) ## grab all columns that start with the letter x
ci_trun <- apply(exp(out_trun[,x.cols]),2,quantile,c(0.025,0.5,0.975))

plot(time,ci_trun[2,],type='n',ylim=range(y_trun,na.rm=TRUE),ylab="Flu Index",log='y',xlim=time[time.rng])
## adjust x-axis label to be monthly if zoomed
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
ecoforecastR::ciEnvelope(time,ci_trun[1,],ci_trun[3,],col="grey")
ecoforecastR::ciEnvelope(time,ci[1,],ci[3,],col="lightBlue")
ecoforecastR::ciEnvelope(time,ci_all[1,],ci_all[3,],col="blue")
points(time, y_all, pch='+', cex=0.5, col = 'red')
points(time,y,pch="+",cex=0.5)
